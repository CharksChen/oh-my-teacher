...
- `core`: 默认层，控制在一个认知视窗内；优先表格、步骤、例题或一张图。
- `expand`: 用户要求深入、需要证明细节或反馈显示仍不理解时再展开。
- `train`: 解释后进入主动回忆、题目、批改或修复。
- 长任务按模块逐段交付，并在每段后只要求一个明确动作。
- 图示仅在流程、结构、空间关系、对比或动态变化明显受益时使用，不强制生成视觉资产。

### Scaffolding Fading

对同一主题按 `teach -> guide -> prompt -> test` 转移责任。具体状态和升降规则见 `references/adaptive-state.md`。

- `teach`: 紧凑讲解并示范一个关键步骤。
- `guide`: 使用针对性问题和提示阶梯，保留关键空白。
- `prompt`: 只提示方法识别、条件或检查点。
- `test`: 先让学生独立完成，再批改。
